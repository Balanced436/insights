...
describe('CRUD operations for summary', () => {
	...
	beforeAll(async () => {
		...
		transcription = await request(app).post('/transcription').send({ sourceId: source.body.data.id }).send({ skipTranscription: true });
	});
	...
	describe('POST summary', () => {
		...
		it("should return 400 with error 'Internal Server Error' and details 'transcriptionId must be provided' when transcriptionId is not provided", async () => {
			const summary = await request(app).post('/summary').send({ content: 'Custom content' });
	...
	describe('GET summary (id)', () => {
		...
		it('should returns summaries by transcriptionid when search param transcriptionid is provided', async () => {
			...
			const s = await prisma.summary.findMany({
				where: { transcriptionId: transcription.body.task.transcriptionId },
			});

			const response1 = await request(app).get(`/summary?transcriptionid=${transcription.body.task.transcriptionId}`);
     ...
     summaries with associated with it 202`, async () => {
			...
			const response = await request(app).get(`/summary?transcriptionid=99`);

			// TODO: Fix summary router to return this status code
