...
const CorpusInfosPage = () => {
	...
	const handleAddSourceClick = () => navigate({ to: `/corpora/${corpusid}/sources/add` });
	...
	if (sources) {
		return (
			<Box sx={{ p: '1em', border: '1px solid red' }}>
				...
				<CorpusSearchBar onAddSourceClick={handleAddSourceClick} onSearchBarChange={() => {}} onDateChange={() => {}} />
				...
				<Box>
					...
					<SourcesGridView sources={sources} onSourceSelection={handleSourceSelection} />
				</Box>
			</Box>
		);
	}
